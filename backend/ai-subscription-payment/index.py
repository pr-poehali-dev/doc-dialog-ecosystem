...
def handler(event: dict, context) -> dict:
    """Создание платежа для подписки на AI-тарифы"""
    ...
    try:
        ...
        if not plan or not amount:
            return {
                'statusCode': 400,
                'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                'body': json.dumps({'error': 'Укажите тариф и сумму'}),
                'isBase64Encoded': False
            }
        
        # Тарифные планы
        plans_config = {
            'basic': {'name': 'Базовый', 'operations': 20, 'price': 299},
            'pro': {'name': 'Профи', 'operations': 100, 'price': 599},
            'unlimited': {'name': 'Безлимит', 'operations': -1, 'price': 999}
        }
        
        if plan not in plans_config or amount != plans_config[plan]['price']:
            return {
                'statusCode': 400,
                'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                'body': json.dumps({'error': 'Неверный тариф или сумма'}),
                'isBase64Encoded': False
            }
        ...
        payment_data = {
            ...
            'metadata': {
                'user_id': str(user_id),
                'plan': plan,
                'operations': str(plan_info['operations']),
        ...
        """, (
            ...
            'ai_subscription', 
            'pending', 
            json.dumps({'plan': plan, 'operations': plan_info['operations']})
        ))
        conn.commit()
