...
const MasseursDirectory = () => {
  ...
  const totalPages = Math.ceil(filteredMasseurs.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentMasseurs = filteredMasseurs.slice(startIndex, endIndex);

  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, selectedCity, selectedSpecialization, sortBy]);
        ...
        {pages.map((page, index) => (
          page === -1 ? (
            ...
            <Button
              ...
              className={currentPage === page ? "" : "hover:bg-secondary"}
            >
              {page}
            </Button>
          )
        ...
        {currentMasseurs.length === 0 ? (
          ...
          <>
            <div className="flex justify-between items-center mb-4 text-sm text-muted-foreground">
              <span>Показано {startIndex + 1}-{Math.min(endIndex, filteredMasseurs.length)} из {filteredMasseurs.length}</span>
              <span>Страница {currentPage} из {totalPages}</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {currentMasseurs.map((masseur) => {
