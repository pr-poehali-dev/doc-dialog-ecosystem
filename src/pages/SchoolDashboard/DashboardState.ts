...
export function useDashboardState(toast: any) {
  ...
  const [activeTab, setActiveTab] = useState<'courses' | 'masterminds' | 'offline-training' | 'promo-requests' | 'subscription' | 'landing-order' | 'webinar-order'>('courses');
  ...
  useEffect(() => {
    ...
    if (tabParam && ['courses', 'masterminds', 'offline-training', 'promo-requests', 'subscription', 'landing-order', 'webinar-order'].includes(tabParam)) {
